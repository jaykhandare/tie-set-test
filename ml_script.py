...
class ExecuteAlgorithm():
    ...
    def __init__(self, project_name, batch_size=100):
        ...
        self.db_object = Sql_database()

    def train_algorithm(self, learning_rate, dataset="./data", epochs=25):
...
class Report():
    ...
    def generate_report(self, project_name, output_path="./reports", format="json"):
        if format == "json":
            data_as_dict = self.db_object.retrieve_data_for_project(project_name)
            # json_obj = json.dumps(data_as_dict, indent=4)        
